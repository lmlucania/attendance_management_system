...
class TemplateView(LoginRequiredMixin, generic.TemplateView):
    ...
    def get_context_data(self):
        ...
        if 'success' in self.request.session:
            context['success'] = self.request.session.pop('success')
        elif 'error' in self.request.session:
            context['error'] = self.request.session.pop('error')
        elif 'warning' in self.request.session:
            context['warning'] = self.request.session.pop('warning')
...
class TimeCardBaseMonthlyReportView(TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        self.EOM_by_url = self.get_EOM_by_url(request)

        return super().dispatch(request, *args, **kwargs)

    def get(self, request, *args, **kwargs):
        if self.request.GET.get('mode') == 'promote':
    ...
    def _get_monthly_report(self, monthly_stamps_qs):
        ...
        for day_index in range(day_count):
            ...
            monthly_report.append({'day': day, 'start_work': start_work, 'end_work': end_work,
                                   'enter_break': enter_break, 'end_break': end_break})
...
class HandleExcelView(View):
    ...
    def _edit_appearance_ws(self, ws):
        ws.merge_cells(start_row=self.HEADER_ROW_NUM, start_column=self.DAY_COL_NUM,
        ...
        for row in ws:
            ...
            for cell in row:
                ...
                if type(cell.value) == str and (self.SHEET_HEADLINE in cell.value or self.SHEET_USER_NAME in cell.value):
