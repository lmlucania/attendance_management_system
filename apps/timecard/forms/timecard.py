...
class BaseTimeCardFormSet(BaseModelFormSet):
    ...
    def __init__(self, *args, **kwargs):
        if kwargs == {}:
            ...
            super().__init__(*args, **kwargs)
            self.queryset = TimeCard.objects.filter(stamped_time__gte=date, stamped_time__lt=(date + relativedelta(
                days=1)), user=user, state=TimeCard.State.NEW).order_by('kind')
